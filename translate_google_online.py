"""
Translate all remaining English TOEIC/TOEFL questions and explanations to Japanese
using Google Translate via the googletrans library (online service).

This will handle all 335 cases where the dictionary-based translation didn't apply.
"""
...
def translate_text(text, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Returns Translated object with .text attribute
...
def process_file(path):
    ...
    for i, q in enumerate(data.get('questions', [])):
        # Question - translate if still in English (matches the english question)
        jq = q.get('ja_question', '')
        orig_q = q.get('question', '')
        
        # If ja_question matches the English question (not translated), translate it
        if jq == orig_q:
            try:
        ...
        # Explanation - translate if still in English
...
def main():
    ...
    print("=" * 70)
    print("Translating remaining English fields to Japanese (Google Translate)")
    print("=" * 70)
